...
public class P453_MinimumMovesToEqualArrayElements {
	 ...
	 * 1. 次数结果最关键的取决于最小的数字和最大的数字。比如，min,a,b,...z,max的结果为N，则min,a+1,b,...z,max结果为N+1, min,a+2,b+1,...z,max结果为N+1+2=N+3。。。。即如果知道了min,a,b,...z,max的结果，则无论a到z怎么变化（在min max范围内），都可以根据N推导出来结果。
	 * 2. min, max, max，... max，假设这里只有min和max，其中max出现的次数为n次，则返回结果为n * (max - min)
	 * 则基于1和2 ，能计算出任意序列的结果。 Example: {3,33,67,100})) result is 191, 推导： (100-3)*3 + (33-100) + (67-100)
	 * @param nums
	 * @return
	 */
	public int minMoves(int[] nums) {
		...
		for (int i = 0; i < nums.length; i++) {
			if(nums[i] != min)
				...
				if(!meetMin){	//in case there're several min values. Only ONE min doesn't involve in baseResult adjustment.
					meetMin = true;
					continue;
				}
				baseResult += (nums[i] - max);
